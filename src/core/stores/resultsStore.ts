...
const useResultsStore = create<ResultsStoreValues & ResultsStoreActions>(((set) => ({
    ...initialData,

    setResultData: (resultData: ResultData) => set(() => (
        {
            resultData: resultData
        }
    )),

    setStore: (storeData: ResultsStoreValues) => set(() => (
        {
            id: storeData.id, 
